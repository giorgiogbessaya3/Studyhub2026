...
<div class="row">
    <!-- Formulaire de modification (gauche) -->
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body p-4">
                <form action="{{ route('admin.chapitres.update', $chapitre) }}" method="POST" id="chapitreForm">
                    @csrf
                    @method('PUT')
                        ...
                        <select name="classe_id" id="classeSelect" class="form-select @error('classe_id') is-invalid @enderror" required>
                            ...
                            @foreach($classes as $classe)
                                <option value="{{ $classe->id }}" 
                                        data-matieres="{{ json_encode($classe->matieres->pluck('nom', 'id')) }}"
                                        {{ old('classe_id', $chapitre->classe_id) == $classe->id ? 'selected' : '' }}>
                                    {{ $classe->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="alert alert-info d-flex align-items-center">
                        ...
                        <div>
                            <small>Slug: <code>{{ $chapitre->slug }}</code></small><br>
                            <small>Créé le: {{ $chapitre->created_at->format('d/m/Y') }}</small>
                        </div>
                    </div>

                    <!-- Boutons -->
                    <div class="d-flex justify-content-between pt-3 border-top">
                        <a href="{{ route('admin.chapitres.index') }}" class="btn btn-light">
    ...
    <!-- Sidebar : Contenus du chapitre (droite) -->
    <div class="col-lg-4">
                        ...
                        <div class="list-group-item p-3">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <div>
                                    ...
                                    <strong class="small">{{ Str::limit($contenu->titre, 25) }}</strong>
                                </div>
        ...
        <div class="card border-0 shadow-sm mt-3">
            <div class="card-body p-3">
                ...
                <div class="d-flex justify-content-between small mb-1">
                    <span>Contenus:</span>
                    <strong>{{ $chapitre->contenus ? $chapitre->contenus->count() : 0 }}</strong>
                </div>
                <div class="d-flex justify-content-between small">
                    <span>Total exercices:</span>
                    <strong>
                        {{ $chapitre->contenus ? $chapitre->contenus->sum(function($c) { 
...
function updateMatieres() {
    ...
    // Récupérer les matières depuis data attribute
    const matieresData = selectedOption.getAttribute('data-matieres');
    const matieres = JSON.parse(matieresData);
