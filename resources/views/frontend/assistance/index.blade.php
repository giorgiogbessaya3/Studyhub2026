...
@section('title', 'Assistance pédagogique - StudyHub')
    ...
    <div class="container mx-auto px-4 relative z-10 py-8">
        <div class="text-center max-w-3xl mx-auto">
            ...
            <p class="text-white/80 text-base md:text-lg">
                La communauté et nos enseignants vous répondent rapidement
            </p>
        </div>
    </div>
...
<section class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-gray-500">Gérez toutes vos questions</p>
            </div>
        </div>
        <a href="{{ route('assistance.create') }}" 
           class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-xl font-medium shadow-lg hover:shadow-xl transition-all">
    ...
    @php
        $mesQuestions = App\Models\Question::where('user_id', Auth::id())
                        ->with(['classe', 'matiere'])
                        ->latest()
                        ->paginate(12);
    @endphp
    
    @if($mesQuestions->isNotEmpty())
    <!-- Grille des questions -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
        @foreach($mesQuestions as $index => $question)
        @php
            // Couleurs par classe - comme dans la page d'épreuves
            $colors = [
        ...
        <div class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden hover:-translate-y-2" 
            ...
            <!-- En-tête avec dégradé - comme la page d'épreuves -->
            ...
            <div class="p-4">
                ...
                <p class="text-xs text-gray-600 mb-4 line-clamp-2">
                    {{ Str::limit(strip_tags($question->contenu), 60) }}
                </p>
    ...
    <div class="text-center py-16">
        ...
        <p class="text-gray-500 mb-6">Vous n'avez pas encore posé de question</p>
        <a href="{{ route('assistance.create') }}" 
...
<section class="max-w-7xl mx-auto px-4 py-16">
    <div class="bg-gradient-to-r from-primary-600 to-primary-700 rounded-2xl p-8 md:p-12 text-center text-white">
        ...
        <h2 class="text-2xl font-bold mb-3">Vous avez une question ?</h2>
        <p class="mb-6 opacity-90 max-w-lg mx-auto">Connectez-vous pour poser vos questions et obtenir des réponses</p>
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="{{ route('login') }}" 
               class="bg-white text-primary-600 hover:bg-gray-100 px-8 py-3 rounded-xl font-medium">
                Se connecter
            </a>
            <a href="{{ route('register') }}" 
               class="bg-yellow-400 text-slate-900 hover:bg-yellow-500 px-8 py-3 rounded-xl font-medium">
                Créer un compte
            </a>
        </div>
    </div>
